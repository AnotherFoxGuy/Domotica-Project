...
#ifdef AREST_PARAMS_MODE
                        uint16_t eq_position = answer.indexOf('=',
                                                              header_length); // Replacing 'magic number' 8 for fixed location of '='
                        if (eq_position != -1)
...
// Modifies arguments in place
    void urldecode(String &arguments) {
        ...
        for (int i = 0; i < arguments.length(); i++) {
            ...
            if ((arguments[i] == '%') && ((a = arguments[i + 1]) && (b = arguments[i + 2])) &&
...
// Add to output buffer
    ...
    void addToBuffer(T toAdd, bool quotable = false) {
        addStringToBuffer(String(toAdd).c_str(),
                          false);   // Except for our overrides, this will be adding numbers, which don't get quoted
    }
